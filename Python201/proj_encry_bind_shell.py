...
class AESCipher:
    def __init__(self, key=None) -> None:
        self._key = key if key else get_random_bytes(32)
        self._cipher = AES.new(self._key, AES.MODE_ECB)

    def encrypt(self, plaintext):
        return self._cipher.encrypt(pad(plaintext, AES.block_size)).hex()

    def decrypt(self, encrypted):
        return unpad(self._cipher.decrypt(bytearray.fromhex(encrypted)), AES.block_size)

    def __str__(self) -> str:
        return "Key -> {}".format(self._key.hex())
...
if args.connect and not args.key:
    parser.error("-c CONNECT requries -k KEY")
